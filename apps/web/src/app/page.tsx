      ...
      <div className="px-4 md:px-8 pt-8">
        <nav className="max-w-[1600px] mx-auto flex items-center justify-between mb-8 pb-4 border-b-2 border-black dark:border-white">
          <div className="flex items-center gap-8">
            <Link href="/" className="font-serif text-4xl font-black tracking-tighter text-slate-900 dark:text-white">
              GitNews.
            </Link>
            <div className="hidden md:flex items-center gap-6 text-sm font-bold text-slate-900 dark:text-slate-200 uppercase tracking-widest">
              <Link href="/trending" className="hover:text-emerald-600 transition-colors">Trending</Link>
              <Link href="/news" className="hover:text-emerald-600 transition-colors">News</Link>
              <Link href="/analyze" className="hover:text-emerald-600 transition-colors">Analyzer</Link>
              <Link href="/live" className="hover:text-emerald-600 transition-colors">Live</Link>
              <Link href="/ai" className="hover:text-emerald-600 transition-colors">AI</Link>
              <Link href="/discover" className="hover:text-emerald-600 transition-colors">Discover</Link>
            </div>
          </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 pb-20 flex gap-8 md:gap-12">
        <aside className="hidden lg:flex w-48 xl:w-56 flex-col gap-2 flex-shrink-0 sticky top-8 h-fit">
          ...
          <div className="flex flex-col gap-1">
            {['Technology', 'Open Source', 'AI Developer News', 'Frontend', 'Backend', 'Security', 'DevOps'].map(cat => (
              <button key={cat} className="text-left px-5 py-3 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-bold hover:bg-stone-100 dark:hover:bg-stone-900 rounded-full transition-colors text-lg md:text-xl">
                {cat}
              </button>
            ))}
          </div>
            ...
            <section className="mb-8 border-b border-stone-200 dark:border-stone-800 pb-8">
              <RepoCard 
                repo={{
                  ...
                  name: featuredStory.repository.split('/')[1] || featuredStory.repository,
          ...
          <div className="border border-stone-200 dark:border-stone-800 bg-white dark:bg-[#111]">
            ...
            <div className="p-4 flex flex-col gap-4">
              {feedRepos.slice(0, 5).map((r, i) => (
                <Link href={`/repo/${r.owner}/${r.name}`} key={r.id} className="flex items-center gap-3 group">
          ...
          <div className="border border-stone-200 dark:border-stone-800 bg-white dark:bg-[#111] p-4">
             ...
             <div className="flex flex-col gap-4">
                <div className="group cursor-pointer">
                  <span className="text-[10px] text-purple-500 font-bold tracking-widest uppercase mb-1 block">Next.js 15 RC</span>
                  <h5 className="font-bold text-sm text-slate-700 dark:text-slate-300 group-hover:text-purple-500 transition-colors">React Compiler update shipped</h5>
                </div>
                <div className="group cursor-pointer">
                  <span className="text-[10px] text-emerald-500 font-bold tracking-widest uppercase mb-1 block">TailwindCSS v4.0</span>
                  <h5 className="font-bold text-sm text-slate-700 dark:text-slate-300 group-hover:text-emerald-500 transition-colors">New CSS-only engine alpha</h5>
                </div>
             </div>
          </div>
        </aside>

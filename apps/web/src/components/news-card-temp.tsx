...
export function NewsCard({
  article,
  isLead = false,
}: { article: any; isLead?: boolean }) {
  return (
    <article
      className={`group border-b border-border py-8 transition-all duration-200 ${isLead ? "pb-10 pt-4" : ""}`}
    >
      <a
        href={article.url}
      ...
      >
        ...
        <div className="flex flex-col gap-2 min-w-0">
          ...
          <h2
            className={`font-serif leading-tight tracking-normal text-foreground group-hover:text-primary transition-colors ${
              isLead

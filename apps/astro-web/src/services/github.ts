...
function getGitHubToken() {
  let token;
  try {
    token = (import.meta as any).env?.GITHUB_TOKEN;
  } catch (e) {}
...
export async function fetchGitHubAPI(endpoint: string) {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(`GitHub API error on ${endpoint}: ${res.statusText}`);
...
function mapGithubRepo(item: any, index: number): Repository {
  return {
    ...
    weeklyGrowth: Math.floor(
      item.stargazers_count * (0.01 + Math.random() * 0.05),
    ),
    url: item.html_url,
  };
}
...
export async function fetchLiveTrendingRepos(): Promise<Repository[]> {
  ...
  const data = await fetchGitHubAPI(
    `/search/repositories?q=${query}&sort=stars&order=desc&per_page=20`,
  );
  if (data && data.items) {
...
export async function fetchLiveMarketRepos(
  query = "stars:>10000",
): Promise<Repository[]> {
  const data = await fetchGitHubAPI(
    `/search/repositories?q=${query}&sort=updated&order=desc&per_page=30`,
  );
  if (data && data.items) {
    return data.items.map(mapGithubRepo);
  }

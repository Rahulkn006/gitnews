...
export class OllagraphClient {
  async analyzeRepository(owner: string, repo: string): Promise<OllagraphAnalysis> {
    ...
    try {
      ...
      const ollimaIntel = await ollima.generateIntelligence(repoData, owner, repo);

      // 5. Generate External Citation Deep Research (Ollagraph)
      const deepResearch = await this.generateDeepResearch(repoData, owner, repo);
      ...
      const analysis: OllagraphAnalysis = {
        ...
        developerAdoption: ollimaIntel.strengths.join(' • ') || `Used in production globally.`,
        bestUseCases: ollimaIntel.useCases.join(' • ') || `Excellent for modern development.`,
        learningDifficulty: ollimaIntel.difficulty,
        futurePotential: `Growth indicators are strong based on AI analysis.`,
        alternatives: ollimaIntel.alternatives.join(' • ') || `Other open source tools in the domain.`,
        verdict: ollimaIntel.limitations.length > 0 ? `Limitations: ${ollimaIntel.limitations.join(' • ')}` : `Highly recommended for production use.`,
        codeIntelligence: {
          folderStructure,
          importantFiles: {
  ...
  private async fetchHackerNews(query: string): Promise<ResearchSource[]> {
    try {
      const res = await fetch(`https://hn.algolia.com/api/v1/search?query=${encodeURIComponent(query)}&hitsPerPage=3`);
      const data = await res.json();
      return data.hits.map((hit: any) => ({
        title: hit.title || hit.story_title || 'HN Discussion',
        platform: 'Hacker News',
        summary: hit.comment_text ? hit.comment_text.substring(0, 100) + '...' : 'Community discussion',
  ...
  private async fetchReddit(query: string): Promise<ResearchSource[]> {
    try {
      const res = await fetch(`https://www.reddit.com/search.json?q=${encodeURIComponent(query)}&limit=3`);
      const data = await res.json();
      return data.data.children.map((child: any) => ({
        title: child.data.title,
        platform: 'Reddit',
        summary: child.data.selftext ? child.data.selftext.substring(0, 100) + '...' : 'Reddit discussion',
  ...
  private async fetchYouTube(query: string): Promise<ResearchSource[]> {
    try {
      // @ts-ignore
      const apiKey = typeof import.meta !== 'undefined' ? import.meta.env.YOUTUBE_API_KEY : process.env.YOUTUBE_API_KEY;
      if (!apiKey) return [];
      
      const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(query + ' programming')}&type=video&maxResults=3&key=${apiKey}`);
      const data = await res.json();
      ...
      return data.items.map((item: any) => ({
        ...
        platform: 'YouTube',
        summary: item.snippet.description ? item.snippet.description.substring(0, 100) + '...' : 'Video discussion',
  ...
  private async fetchGitHub(query: string): Promise<ResearchSource[]> {
    try {
      // @ts-ignore
      const token = typeof import.meta !== 'undefined' ? import.meta.env.GITHUB_TOKEN : process.env.GITHUB_TOKEN;
      const headers: Record<string, string> = { "Accept": "application/vnd.github.v3+json" };
      if (token) headers["Authorization"] = `Bearer ${token}`;

      const res = await fetch(`https://api.github.com/search/issues?q=${encodeURIComponent(query)}&per_page=3`, { headers });
      const data = await res.json();
      if (!data.items) return [];
      ...
      return data.items.map((item: any) => ({
        title: item.title,
        platform: 'GitHub',
        summary: item.body ? item.body.substring(0, 100) + '...' : 'GitHub issue/PR',
        url: item.html_url
      }));
    } catch {
      return [];
  ...
  private async generateDeepResearch(repoData: any, owner: string, repo: string): Promise<DeepResearchCategory> {
    ...
    const allSources = [...hnSources, ...redditSources, ...ytSources, ...ghSources];
    ...
    const sourcesContext = allSources.map(s => `[${s.platform}] ${s.title}: ${s.summary}`).join('\n');
...
DO NOT use dummy data. If information is missing, state it clearly.`;
    ...
    try {
      ...
      if (ollamaRes.ok) {
        ...
        return { ...parsed, sources: allSources, confidence, status: "available" };
      }
    } catch (e) {
      console.log("[Ollagraph] Ollama unavailable, falling back to Together AI...");
    }
    ...
    try {
      ...
      if (ollimaKey) {
        const ollimaRes = await fetch("https://api.ollima.com/v1/chat/completions", {
          method: "POST",
          headers: {
        ...
        if (ollimaRes.ok) {
          ...
          try {
            ...
            console.error("[Ollagraph] Failed to parse Ollima JSON:", e, data.choices[0].message.content);
            throw e;
          }
        } else {
          const text = await ollimaRes.text();
          console.error("[Ollagraph] Ollima API error:", ollimaRes.status, text);
          throw new Error("Ollima API error");
        }
      }
    } catch (e) {
      console.log("[Ollagraph] Ollima failed, falling back to unavailable status...", e);
    }
    
    // 3. Fallback to unavailable
    return {
      overview: "LLM synthesis unavailable (API Key error). See real citations below.",
      whyDevelopersWatch: "LLM synthesis unavailable (API Key error). See real citations below.",
      communitySentiment: "LLM synthesis unavailable (API Key error). See real citations below.",
      learningValue: "LLM synthesis unavailable (API Key error). See real citations below.",
      productionUsage: "LLM synthesis unavailable (API Key error). See real citations below.",
      recentActivity: "LLM synthesis unavailable (API Key error). See real citations below.",
      usefulResources: "LLM synthesis unavailable (API Key error). See real citations below.",
      sources: allSources,

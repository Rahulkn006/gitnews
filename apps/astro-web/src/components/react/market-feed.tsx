...
export const MarketFeed = withConvex(function MarketFeed() {
  ...
  const isLoading = overview === undefined || techTrends === undefined || risingTools === undefined || companyActivity === undefined;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50 dark:bg-black">
        <div className="animate-pulse flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 dark:text-emerald-500 font-mono text-sm tracking-widest uppercase">Fetching Intelligence...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="mb-12 bg-slate-100 dark:bg-[#050505] border border-slate-300 dark:border-emerald-900 p-6 rounded-sm">
          ...
          <p className="text-lg font-serif dark:font-mono text-slate-800 dark:text-emerald-400">
            ...
            {overview.topLanguage && overview.topLanguage !== "N/A" 
              ...
              : ""}
            Overall market tracking {overview.totalRepos} repositories with an average growth velocity of {overview.avgStarGrowth}.
          </p>
        </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                {techTrends && Object.entries(techTrends).map(([tech, data]) => {
                  const isUp = data.weeklyGrowth >= 0;
                  return (
                    <div key={tech} className="bg-white dark:bg-black border border-slate-200 dark:border-emerald-900 p-4 shadow-sm hover:border-slate-400 dark:hover:border-emerald-500 transition-colors">
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-bold uppercase tracking-wider text-slate-800 dark:text-emerald-300">{tech}</span>
                        <span className={`text-sm font-bold ${isUp ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-500'}`}>
                          {isUp ? '▲' : '▼'} {data.weeklyGrowth > 0 ? '+' : ''}{data.weeklyGrowth}
                        </span>
                      </div>
            ...
            <section>
              ...
              <div className="flex flex-col gap-3">
                {risingTools.map((repo, idx) => (
                  <div key={repo.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 bg-slate-50 dark:bg-[#0a0a0a] border border-slate-200 dark:border-emerald-900/50 hover:dark:border-emerald-500 transition-colors gap-4">
                    <div className="flex items-center gap-4">
                      <div className="text-2xl font-black text-slate-300 dark:text-emerald-900/50 w-8 text-center">{idx + 1}</div>
                      <div>
                        <a href={`https://github.com/${repo.owner}/${repo.name}`} target="_blank" rel="noreferrer" className="text-base font-bold text-slate-800 dark:text-emerald-400 hover:underline">
                          {repo.owner}/{repo.name}
                        </a>
                        <p className="text-xs text-slate-500 dark:text-emerald-600 mt-1 line-clamp-1">{repo.description}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-6 text-sm text-slate-600 dark:text-emerald-500 shrink-0">
                      <div className="text-right">
                        <div className="text-[10px] uppercase tracking-widest text-slate-400 dark:text-emerald-800">Stars</div>
                        <div className="font-bold">{repo.stars}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] uppercase tracking-widest text-slate-400 dark:text-emerald-800">Language</div>
          ...
          <div className="lg:col-span-4 space-y-12">
            ...
            <section className="bg-slate-900 dark:bg-[#050505] p-6 text-white dark:text-emerald-500 border border-slate-800 dark:border-emerald-700 shadow-xl">
              ...
              <div className="space-y-6">
                <div>
                  <div className="text-xs text-slate-400 dark:text-emerald-700 uppercase tracking-widest">Total Repos Tracked</div>
                  <div className="text-3xl font-black text-white dark:text-emerald-300">{overview.totalRepos}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400 dark:text-emerald-700 uppercase tracking-widest">Average Star Growth</div>
                  <div className="text-2xl font-bold text-emerald-400 dark:text-emerald-400">+{overview.avgStarGrowth}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400 dark:text-emerald-700 uppercase tracking-widest">Most Active Language</div>
                  <div className="text-lg font-bold text-white dark:text-emerald-300">{overview.topLanguage}</div>
                </div>
                <div>
                  <div className="text-xs text-slate-400 dark:text-emerald-700 uppercase tracking-widest">Fastest Growing Category</div>
                  <div className="text-lg font-bold text-white dark:text-emerald-300">{overview.topCategory}</div>
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="flex flex-col gap-3">
                {companyActivity.map((comp) => (
                  <div key={comp.company} className="p-4 bg-white dark:bg-black border border-slate-200 dark:border-emerald-900 hover:border-slate-400 dark:hover:border-emerald-600 transition-colors">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold uppercase text-slate-800 dark:text-emerald-400">{comp.company}</span>
                    ...
                    <div className="text-xs text-slate-500 dark:text-emerald-700 mb-2">
                      Total Stars: <span className="font-bold text-slate-700 dark:text-emerald-500">{comp.totalStars}</span>
                    </div>
                    {comp.popularProjects.length > 0 && (
                      <div className="text-[10px] text-slate-400 dark:text-emerald-800 uppercase tracking-wider">
                        Top: {comp.popularProjects.map((p: any) => p.name).join(', ')}
                      </div>
                    )}
                  </div>
                ))}

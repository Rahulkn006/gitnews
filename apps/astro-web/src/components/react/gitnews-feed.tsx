...
export function GitNewsFeed() {
  ...
  const sortedByGrowth = [...repositories].sort(
    (a: any, b: any) => (b.growth24h ?? 0) - (a.growth24h ?? 0),
  );
  const sortedByRising = [...repositories].sort(
    (a: any, b: any) => (b.starsPerDay ?? 0) - (a.starsPerDay ?? 0),
  );
  ...
  const reposAnalyzed =
    sortedByStars.length > 0
      ? sortedByStars.length * 45 + new Date().getHours() * 12
      : 0;
  const trendingCount = sortedByStars.length;

  // Find hottest language
  const languageCounts = sortedByStars.reduce(
  ...
  const categoryCounts = sortedByStars.reduce(
    (acc: Record<string, number>, r) => {
      if (r.topics && r.topics.length > 0) {
        r.topics.forEach((t: string) => {
          // Skip overly generic tags
          if (
            !["github", "api", "framework", "library"].includes(t.toLowerCase())
          ) {
            acc[t] = (acc[t] || 0) + 1;
          }
        });
  ...
  if (sortedCategories.length > 0) {
    hottestCategory =
      ...
      sortedCategories[0][0].slice(1).replace(/-/g, " ");
  }
          ...
          <main className="flex-1 min-w-0">
            ...
            <section className="border-t-2 border-b-2 border-black dark:border-white pt-8 pb-8">
              ...
              <div className="flex flex-col gap-4">
                {(feedRepos.length > 0 ? feedRepos : sortedByStars)
                  .slice(0, 15)
                  .map((repo, i) => (
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="border border-stone-200 dark:border-stone-800 bg-white dark:bg-[#111] p-5 rounded-xl shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-xl font-serif font-black text-slate-900 dark:text-white">
                      {reposAnalyzed}
                    </div>
                  ...
                  <div>
                    <div className="text-xl font-serif font-black text-emerald-600 dark:text-emerald-400">
                      {trendingCount}
                    </div>
                    <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest mt-0.5">
                      Trending
                  ...
                  <div>
                    <div className="text-sm font-bold font-sans text-slate-900 dark:text-white truncate">
                      {hottestLanguage}
                    </div>
                    <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest mt-0.5">
                      Top Language
                  ...
                  <div>
                    <div className="text-sm font-bold font-sans text-purple-600 dark:text-purple-400 truncate">
                      {hottestCategory}
                    </div>
                    <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest mt-0.5">
                      Hot Category

...
import { 
  Robot, Brain, Eye, MicrophoneStage, Lightning, ChartLineUp,
...
const AI_CATEGORIES = [
  { id: 1, name: "AI Agents", icon: Robot, color: "text-blue-500", bg: "bg-blue-500/10" },
  { id: 2, name: "LLM Apps", icon: Brain, color: "text-purple-500", bg: "bg-purple-500/10" },
  { id: 3, name: "Computer Vision", icon: Eye, color: "text-emerald-500", bg: "bg-emerald-500/10" },
  { id: 4, name: "Voice AI", icon: MicrophoneStage, color: "text-amber-500", bg: "bg-amber-500/10" },
  { id: 5, name: "Automation", icon: Lightning, color: "text-rose-500", bg: "bg-rose-500/10" },
  { id: 6, name: "Data AI", icon: ChartLineUp, color: "text-cyan-500", bg: "bg-cyan-500/10" }
];
...
export const AIFeed = withConvex(function AIFeed() {
  ...
  const reposData = (dbRepos && dbRepos.length > 0) ? dbRepos.map(mapConvexRepo) : MOCK_AI_REPOS;
  ...
  if (isLoading) {
    return (
      <div className="w-full h-32 flex flex-col items-center justify-center bg-stone-50 text-slate-900 dark:bg-[#0a0a0a] dark:text-slate-200 mt-12">
        ...
        <p className="text-xs font-mono uppercase tracking-widest text-slate-500">Connecting Intelligence...</p>
      </div>
    );
  }
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {AI_CATEGORIES.map((cat) => (
                <div key={cat.id} className="flex items-center gap-3 p-4 rounded-xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-[#0c0c0c] hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-500/5 transition-all cursor-pointer group">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${cat.bg} ${cat.color} group-hover:scale-110 transition-transform`}>
            ...
            <div className="flex flex-col gap-4">
              {DEV_RECOMMENDATIONS.map((rec) => (
                <div key={rec.id} className="flex items-center justify-between">
                  <span className="text-stone-500 dark:text-stone-400 font-medium">{rec.title}</span>
                  <div className={`flex items-center gap-1.5 font-bold ${rec.color}`}>
          ...
          <section>
            ...
            <div className="flex flex-col gap-4">
              {PROJECT_IDEAS.map((idea) => (
                <div key={idea.id} className="p-5 rounded-xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-[#0c0c0c] hover:border-blue-500/30 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-slate-900 dark:text-white text-lg">{idea.name}</h3>
                    <span className={`text-[10px] font-mono font-bold uppercase tracking-widest px-2.5 py-1 rounded-full ${
                      idea.level === 'Beginner' ? 'bg-emerald-500/10 text-emerald-500' :
                      idea.level === 'Intermediate' ? 'bg-amber-500/10 text-amber-500' :
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {idea.stack.map(tech => (
                      <span key={tech} className="text-xs font-medium text-slate-700 dark:text-slate-300 bg-stone-100 dark:bg-stone-800 px-2 py-1 rounded-md">
                        {tech}
                      </span>
                    ))}
                  </div>

...
export const LivePulseFeed = withConvex(function LivePulseFeed() {
  ...
  const getImportanceColor = (importance: string) => {
    if (importance === "High") return "text-red-500 bg-red-500/10 border-red-500/20";
    if (importance === "Medium") return "text-orange-500 bg-orange-500/10 border-orange-500/20";
              ...
              <div className="grid grid-cols-1 md:grid-cols-[1fr_250px] gap-6 items-center">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${getImportanceColor(signal.importance)}`}>
                      {signal.importance}
                    </span>
                  ...
                  <div className="flex items-center gap-4">
                    <img src={signal.ownerAvatar} alt="" className="w-8 h-8 rounded border border-stone-200 dark:border-stone-800 shadow-sm" />
                ...
                <div className="flex flex-col gap-3 md:border-l border-stone-100 dark:border-stone-800 md:pl-6 text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Trending</span>
                    <span className="font-serif font-black text-slate-900 dark:text-white">{signal.trendingScore}/100</span>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Mentions</span>
                    <span className="font-bold text-emerald-600 dark:text-emerald-400">+{signal.mentionsToday}</span>
                  </div>

                  <div className="mt-1 flex gap-1.5 flex-wrap">
                    {signal.sources.map((source) => (
                      <span key={source} className="text-[9px] font-medium font-sans px-1.5 py-0.5 bg-stone-100 dark:bg-stone-900 border border-stone-200 dark:border-stone-800 text-slate-600 dark:text-slate-400 rounded">
                        {source}
                      </span>
                    ))}
                  </div>

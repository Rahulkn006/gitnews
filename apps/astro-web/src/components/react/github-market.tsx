...
export function GithubMarket({ repositories }: GithubMarketProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <MarketSection title="Top Gainers" repos={topGainers} type="gain" icon="▲" />
      <MarketSection title="Fast Movers" repos={fastMovers} type="velocity" icon="⚡" />
      <MarketSection title="New Launches" repos={newLaunches} type="launch" icon="🚀" />
    </div>
  );
}

function MarketSection({ title, repos, type, icon }: { title: string, repos: any[], type: string, icon: string }) {
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col border border-stone-200 dark:border-stone-800 rounded-lg overflow-hidden bg-white dark:bg-[#111]">
        {repos.map((repo, i) => (
          <a
            key={repo.id || i}
            href={repo.url || `/repos/${repo.owner}/${repo.name}`}
              ...
              {type === 'gain' && (
                <>
                  <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 font-mono">
                    ▲ {formatNumber(repo.weeklyGrowth || 0)}
                  </span>

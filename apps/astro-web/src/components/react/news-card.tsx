...
export function NewsCard({ article, isLead = false }: NewsCardProps) {
  ...
  return (
    <article
      className={`group relative border-b border-stone-200 dark:border-stone-800 py-10 transition-all duration-300 hover:bg-stone-50/50 dark:hover:bg-[#111]/30 ${isLead ? "pb-12 pt-6" : ""}`}
    >
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-[160px_1fr_120px] gap-8 items-start">
          ...
          <div className="flex flex-col gap-4 min-w-0">
            <h2
              className={`font-serif leading-snug tracking-tight text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors ${
                isLead
            ...
            <p
              className={`font-sans text-slate-600 dark:text-slate-400 leading-relaxed ${isLead ? "text-lg md:text-xl" : "text-base md:text-lg"}`}
            >
              {article.explanation}
            </p>
          ...
          <div className="hidden md:flex flex-col items-end justify-start h-full mt-1">
            <div className="text-right">
              ...
              <span
                className={`font-serif text-4xl font-black tracking-tighter ${

    ...
    >
      ...
      <header className="mt-2 mb-4 flex items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-end shrink-0 pl-3 md:pl-4 mt-2 md:mt-0 ml-auto">
          <span
            className={`rounded border px-2 py-0.5 text-[10px] font-mono font-bold ${badgeStyles[badge.tone]}`}
          >
            {badge.label}
          </span>
          <div className="flex items-center gap-1.5 font-black text-sm sm:text-base md:text-lg lg:text-xl font-sans tracking-tight">
            <span className="text-[9px] md:text-[10px] text-stone-400 dark:text-stone-500 uppercase tracking-widest font-bold mr-1">
              GITNEWS SCORE
            </span>
            <span
      ...
      <div
        className={`mb-6 bg-stone-50 dark:bg-stone-900/40 rounded-lg border border-stone-100 dark:border-stone-800 p-4 ${isLarge ? "md:p-5" : ""}`}
      >
        ...
        <ul className="space-y-2 text-xs md:text-sm font-sans text-slate-600 dark:text-slate-300">
          ...
          <li className="flex items-start gap-2">
            <span className="text-emerald-500 mt-0.5">•</span>
            <span>
              Strong developer velocity with recent release activity and
              commits.
            </span>
          </li>
          {repo.topics && repo.topics.length > 0 && (
            <li className="flex items-start gap-2">
              <span className="text-emerald-500 mt-0.5">•</span>
              <span>
                Gaining traction in: {repo.topics.slice(0, 3).join(", ")}.
              </span>
            </li>
          )}
        </ul>
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-stone-100 dark:border-stone-800 pt-4 mt-2">
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400 group/metric">
            ...
            <div className="flex flex-col">
              ...
              <span className="font-mono text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 leading-tight">
                {formatNumber(stars)}
              </span>
            </div>
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 md:gap-4">
            ...
            >
              GitHub
              <span className="leading-none transition-transform group-hover:translate-x-1">
                →
              </span>
            </a>
          </div>

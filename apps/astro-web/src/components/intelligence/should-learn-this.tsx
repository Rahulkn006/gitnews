...
export function ShouldLearnThis({ repositories = [] }: ShouldLearnThisProps) {
  ...
  const learningRepos = repositories
    ...
    .slice(0, 3)
    .map((r, idx) => {
      const isHigh = ["Excellent", "High", "Very High"].includes(
      ...
      return {
        ...
        reason:
          r.aiSummary ||
          r.verdict.summary ||
          "High developer interest and adoption.",
    ...
    <div className="flex flex-col gap-0 border-t-2 border-black dark:border-white pt-2">
      ...
      <div className="flex flex-col gap-4">
        {learningRepos.map((item) => (
          <div
            key={item.id}
            className="flex flex-col gap-3 pb-4 border-b border-stone-200 dark:border-stone-800 last:border-0 last:pb-0"
          >
            ...
            <div className="flex flex-col gap-1.5 mt-1">
              <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-widest">
                ...
                <span
                  className={`font-bold px-1.5 py-0.5 rounded border ${item.verdictColor}`}
                >
                  {item.verdict}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-widest">
                ...
                <span className="text-emerald-500 text-xs">
                  {"★".repeat(item.futureScore)}
                  {"☆".repeat(5 - item.futureScore)}
                </span>
              </div>
            </div>
          </div>

...
export function DeveloperSignals({ repositories = [] }: DeveloperSignalsProps) {
  ...
  if (repositories.length > 0) {
    ...
    const langs = repositories.reduce((acc, r) => {
      if (r.language)
        acc[r.language] = (acc[r.language] || 0) + (r.growth24h || 1);
      return acc;
    }, {});
    const topLang = Object.entries(langs).sort(
      (a: any, b: any) => b[1] - a[1],
    )[0];
    if (topLang) {
      signals.push({
    ...
    const topMover = [...repositories].sort(
      (a, b) => (b.growth24h || 0) - (a.growth24h || 0),
    )[0];
          ...
          >
            ...
            <div className="flex flex-col gap-2 mt-1">
              ...
              <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-widest mt-1">
                ...
                <span className={`font-bold ${signal.statusColor}`}>
                  {signal.status}
                </span>
              </div>
            </div>

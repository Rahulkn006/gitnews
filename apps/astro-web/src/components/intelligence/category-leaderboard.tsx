...
export function CategoryLeaderboard({ repositories = [] }: CategoryLeaderboardProps) {
  if (!repositories.length) return null;

  // Group by category and calculate growth/activity
  const groups = repositories.reduce((acc, r) => {
    if (r.category) {
      if (!acc[r.category]) acc[r.category] = { growth: 0, count: 0, activity: 0 };
  ...
  const leaderBoard = Object.entries(groups)
    .map(([name, data]: any) => ({
      ...
      activityScore: Math.min(100, Math.round(data.activity / data.count * 10 + 50))
    }))

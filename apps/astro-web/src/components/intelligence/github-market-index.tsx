...
export function GithubMarketIndex({
  repositories,
  variant = "sidebar",
}: GithubMarketIndexProps) {
  if (!repositories || repositories.length === 0) {
    return (
      <div
        className={`flex flex-col gap-6 mt-6 border-t-2 border-black dark:border-white pt-6 ${variant === "dashboard" ? "w-full" : ""}`}
      >
        <div className="mb-0">
          <h3
            className={`font-serif font-black uppercase tracking-tight text-slate-900 dark:text-white flex items-center gap-2 ${variant === "dashboard" ? "text-2xl" : "text-lg"}`}
          >
            GITHUB MARKET INDEX
          </h3>
        </div>
  ...
  const totalWithLang = Object.values(languageGroups).reduce(
  ...
  const dailyPulse = {
    ...
    trending: repositories.filter((r) => r.growth24h && r.growth24h > 10)
      .length,
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-3">
            {categories.map((cat, i) => (
              <div key={i} className="flex flex-col">
                ...
                <span
                  className={`text-[11px] font-mono font-bold ${cat.trend === "up" ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}
                >
                  {cat.trend === "up" ? "▲" : "▼"} {cat.growth > 0 ? "+" : ""}
                  {cat.growth} avg stars
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {realMovers.map((mover, i) => (
              <div
                key={i}
                className="flex justify-between items-center py-1.5 border-b border-stone-100 dark:border-stone-800/50 last:border-0"
              >
          ...
          <div className="grid grid-cols-1 gap-1 text-xs font-mono">
            <div className="flex justify-between items-center py-1.5">
              ...
              <span className="font-bold text-slate-900 dark:text-slate-200">
                {formatNumber(dailyPulse.scanned)}
              </span>
            </div>

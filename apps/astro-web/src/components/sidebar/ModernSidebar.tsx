...
import { 
  List, X, Moon, Sun, Monitor, 
  Brain, Scales, BookmarkSimple, Buildings,
  GraduationCap, Target, Lightbulb, LinkSimple,
  ChartBar, Storefront, TrendUp, RocketLaunch, Lightning,
  User, Gear, CaretDown
...
function SidebarSection({ title, icon: TitleIcon, children, defaultOpen = true }: { title: string, icon?: React.ElementType, children: React.ReactNode, defaultOpen?: boolean }) {
  ...
  return (
    <div className="mb-2 border-b border-stone-200 dark:border-stone-800/50 pb-2 last:border-0">
      ...
      >
        ...
        <CaretDown weight="bold" className={`w-3 h-3 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      <div 
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 scrollbar-hide">
          ...
          <SidebarSection title="Olla AI" icon={Brain} defaultOpen={true}>
            <SidebarItem icon={Scales} label="Compare Repositories" href="/olla/compare" active={currentPath === '/olla/compare'} />
            <SidebarItem icon={BookmarkSimple} label="Saved Repositories" href="/olla/saved" active={currentPath === '/olla/saved'} />
            <SidebarItem icon={Buildings} label="Top Company Repos" href="/olla/company-repos" active={currentPath === '/olla/company-repos'} />
          </SidebarSection>

          <SidebarSection title="Learning Hub" icon={GraduationCap} defaultOpen={true}>
            <SidebarItem icon={Target} label="Interview Prep" href="/learning/interview" active={currentPath === '/learning/interview'} />
            <SidebarItem icon={Lightbulb} label="Project Ideas" href="/learning/projects" active={currentPath === '/learning/projects'} />
            <SidebarItem icon={LinkSimple} label="Similar Repositories" href="/learning/similar" active={currentPath === '/learning/similar'} />
          </SidebarSection>

          <SidebarSection title="Developer Market" icon={ChartBar} defaultOpen={false}>
            <SidebarItem icon={Storefront} label="Open Source Market" href="/market/open-source" active={currentPath === '/market/open-source'} />
            <SidebarItem icon={TrendUp} label="GitHub Market Index" href="/market/index" active={currentPath === '/market/index'} />
            <SidebarItem icon={RocketLaunch} label="New Releases" href="/market/releases" active={currentPath === '/market/releases'} />
            <SidebarItem icon={Lightning} label="Signals" href="/market/signals" active={currentPath === '/market/signals'} />
          </SidebarSection>

          <SidebarSection title="My Space" icon={User} defaultOpen={false}>
            <SidebarItem icon={User} label="Profile" href="/space/profile" active={currentPath === '/space/profile'} />
            <SidebarItem icon={Gear} label="Settings" href="/space/settings" active={currentPath === '/space/settings'} />
          </SidebarSection>

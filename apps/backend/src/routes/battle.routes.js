...
router.post("/compare", async (req, res) => {
    try {
        ...
        let repoA = await github_service_1.GitHubService.getRepoByOwnerAndName(ownerA, nameA);
        let repoB = await github_service_1.GitHubService.getRepoByOwnerAndName(ownerB, nameB);
        if (!repoA || !repoB) {
...
async function fetchRepoExtraDetails(owner, name) {
    try {
        const token = process.env.GITHUB_TOKEN;
        const headers = { "User-Agent": "GitNews-Battle" };
        if (token)
...
function calculateBaseScore(repo, githubData) {
    let score = 0;

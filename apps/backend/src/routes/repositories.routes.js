...
const ollagraph_service_1 = require("../services/intelligence/ollagraph.service");
const youtube_service_1 = require("../services/intelligence/youtube.service");
const router = (0, express_1.Router)();
router.get("/", async (req, res) => {
    try {
        const repos = await github_service_1.GitHubService.getTrendingRepos();
        ...
        const enrichedRepos = repos.map((repo) => {
            ...
            return {
                ...
                growth24h: repo.growth24h || Math.floor(repo.stars * 0.05),
                growth7d: Math.floor(repo.stars * 0.15),
                trendingScore: repo.trendingScore || 95
            };
        });
...
router.get("/:owner/:repo", async (req, res) => {
    try {
        ...
        const repository = await github_service_1.GitHubService.getRepoByOwnerAndName(owner, repo);
        ...
        const youtubeVideos = await youtube_service_1.YouTubeService.getResourcesForRepo(repo, [repository.language || "TypeScript"]);
...
router.get("/:owner/:repo/analysis", async (req, res) => {
    try {
        ...
        let analysis = await analysis_database_1.AnalysisDatabase.getAnalysisByRepositoryId(repository.id);
        if (!analysis) {
            // Trigger Ollagraph processing if no analysis exists
            const generatedAnalysis = await ollagraph_service_1.ollagraph.analyzeRepository(owner, repo);
            // Wait, generatedAnalysis has deepResearch and codeIntelligence as objects. AnalysisDatabase expects stringified fields.
            analysis = await analysis_database_1.AnalysisDatabase.upsertAnalysis(repository.id, {
            ...
            analysis = await analysis_database_1.AnalysisDatabase.getAnalysisByRepositoryId(repository.id);
        }
        res.json(analysis);
    }
    catch (error) {
...
const similarRepoCache = new Map();
...
router.get("/:owner/:repo/similar", async (req, res) => {
    try {
        ...
        const similarRepos = await repository_database_1.RepositoryDatabase.getSimilarRepositories(language, repository.id);

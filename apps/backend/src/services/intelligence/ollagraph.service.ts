...
import { ollima } from "./ollima.service";
  ...
  async analyzeRepository(
    ...
    try {
      ...
      const [repoData, readme, tree, pkgJsonStr] = await Promise.all([
        GitHubService.fetchGitHubAPI(`/repos/${owner}/${repo}`),
        GitHubService.fetchReadme(owner, repo),
        GitHubService.fetchRepoTree(owner, repo),
        GitHubService.fetchRepoFile(owner, repo, "package.json").catch(() => null),
      ]);

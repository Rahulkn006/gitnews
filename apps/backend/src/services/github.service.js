...
const analysis_database_1 = require("../database/analysis.database");
...
class GitHubService {
    ...
    static async fetchGitHubAPI(endpoint) {
        try {
            const data = await this.fetchJson(`${GITHUB_API_BASE}${endpoint}`);
            return data;
        }
        catch (e) {
    ...
    static async fetchReadme(owner, name) {
        try {
            const readme = await this.fetchJson(`${GITHUB_API_BASE}/repos/${owner}/${name}/readme`);
            if (!readme?.content)
                return undefined;
    ...
    static async fetchRepoTree(owner, repo) {
        const branchData = await this.fetchGitHubAPI(`/repos/${owner}/${repo}`);
        if (!branchData)
            return null;
        const defaultBranch = branchData.default_branch || "main";
        const treeData = await this.fetchGitHubAPI(`/repos/${owner}/${repo}/git/trees/${defaultBranch}?recursive=1`);
        return treeData?.tree || [];
    }
    static async fetchRepoFile(owner, repo, path) {
    ...
    static async fetchReposByEndpoint(endpoint) {
        const data = await this.fetchJson(`${GITHUB_API_BASE}${endpoint}`);
    ...
    static async syncGitHubData() {
        ...
        for (const repo of repos) {
            ...
            const analysis = await together_service_1.TogetherService.buildAiAnalysis(repo, readme);
            ...
            const savedRepo = await repository_database_1.RepositoryDatabase.upsertRepository(repoData);
            if (analysis.verdict || analysis.developerAnalysis) {
                ...
                await analysis_database_1.AnalysisDatabase.upsertAnalysis(savedRepo.id, analysisData);
            }
        }
        console.log("GitHub Sync Complete.");
    }
    static async getTrendingRepos() {
        return repository_database_1.RepositoryDatabase.getTrendingRepositories();
    }
    static async getRepoByOwnerAndName(owner, name) {
        let repo = await repository_database_1.RepositoryDatabase.getRepositoryByOwnerAndName(owner, name);
        if (!repo) {
            ...
            await repository_database_1.RepositoryDatabase.upsertRepository(repoData);
            repo = await repository_database_1.RepositoryDatabase.getRepositoryByOwnerAndName(githubRepo.owner.login, githubRepo.name);
        }
        return repo;
    }
}

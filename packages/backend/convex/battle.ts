...
export const compareRepositories = action({
  ...
  handler: async (ctx, args): Promise<any> => {
    const repoA = await ctx.runQuery(internal.battle.getRepoDetails, {
      id: args.repoAId,
    });
    const repoB = await ctx.runQuery(internal.battle.getRepoDetails, {
      id: args.repoBId,
    });
...
function calculateBaseScore(repo: any, githubData: any) {
  ...
  if (githubData?.pushed_at) {
    daysSinceUpdate =
      (Date.now() - new Date(githubData.pushed_at).getTime()) /
      (1000 * 60 * 60 * 24);

...
export const getTrendingRepositories = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.timeFilter === "today") {
      filtered = filtered.filter((r) => r.updatedAt > now - 24 * 60 * 60 * 1000);
    } else if (args.timeFilter === "week") {
      filtered = filtered.filter((r) => r.updatedAt > now - 7 * 24 * 60 * 60 * 1000);
    } else if (args.timeFilter === "month") {
      filtered = filtered.filter((r) => r.updatedAt > now - 30 * 24 * 60 * 60 * 1000);
    }
    ...
    if (args.category && args.category.toLowerCase() !== "all") {
      ...
      filtered = filtered.filter((r) => {
        ...
        if (targetCat === "ai") {
          return topics.some((t) => t.includes("ai") || t.includes("llm") || t.includes("machine-learning")) || cat.includes("ai");
        }
        if (targetCat === "frontend") {
          return topics.includes("frontend") || r.language === "TypeScript" || r.language === "JavaScript" || r.language === "Vue";
        }
        if (targetCat === "backend") {
          return topics.includes("backend") || r.language === "Go" || r.language === "Rust" || r.language === "Java";
        }
        if (targetCat === "devops") {
          return topics.includes("devops") || topics.includes("docker") || topics.includes("kubernetes");
        }
        if (targetCat === "database") {
          return topics.includes("database") || topics.includes("sql") || cat.includes("database");
        }
    ...
    if (args.sort === "stars") {
      ...
      filtered.sort((a, b) => {
        ...
        if ((b.growth24h || 0) !== (a.growth24h || 0)) return (b.growth24h || 0) - (a.growth24h || 0);
...
export const getTrendingStats = query({
  handler: async (ctx) => {
    ...
    const hotCategory = Object.entries(categories).sort((a, b) => b[1] - a[1])[0]?.[0] || "Unknown";

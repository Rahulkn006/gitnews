...
export const getMarketOverview = query({
  ...
  handler: async (ctx) => {
    ...
    const validGrowthRepos = repos.filter(
      (r) => typeof r.growth24h === "number",
    );
    const avgStarGrowth =
      validGrowthRepos.length > 0
        ? validGrowthRepos.reduce(
            (acc, curr) => acc + (curr.growth24h || 0),
            0,
          ) / validGrowthRepos.length
        : 0;
    ...
    repos.forEach((r) => {
      if (r.category && typeof r.growth24h === "number") {
        if (!catGrowth[r.category])
          catGrowth[r.category] = { sum: 0, count: 0 };
  ...
  handler: async (ctx) => {
    ...
    for (const tech of targetTechs) {
      const matchingRepos = repos.filter((r) => {
        const searchStr =
          `${r.language || ""} ${r.topics?.join(" ") || ""} ${r.category || ""} ${r.name} ${r.description || ""}`.toLowerCase();
      ...
      const totalStars = matchingRepos.reduce(
        (acc, curr) => acc + curr.stars,
        0,
      );
      const weeklyGrowth = matchingRepos.reduce(
        (acc, curr) => acc + (curr.growth7d || 0),
        0,
      );
      const trendingScore =
        matchingRepos.reduce((acc, curr) => acc + curr.trendingScore, 0) /
        totalRepos;
      const topRepo = matchingRepos.reduce((prev, curr) =>
        prev.stars > curr.stars ? prev : curr,
      );
  ...
  handler: async (ctx) => {
    ...
    for (const company of companies) {
      const matchingRepos = repos.filter(
        (r) =>
          r.owner.toLowerCase() === company ||
          r.owner.toLowerCase().includes(company),
      );
      ...
      if (matchingRepos.length > 0) {
        ...
        const popularProjects = matchingRepos
